...
class _GameState extends State<Game> {
  ...
  _generateQoute() async {
    var response = await rootBundle.loadString('json_file/kuis.json');
    int acak = random.nextInt(json.decode(response).length);
    var object = json.decode(response)[acak]['question'];

    var kunci = json.decode(response)[acak]['key'];

    return setState(() {
      mylist = [
        kunci,
        json.decode(response)[acak]['A'],
        json.decode(response)[acak]['B'],
        json.decode(response)[acak]['C'],
      ];
      mylist.shuffle();
      soal = soal + 1;
  ...
  _jawaban(jwb) async {
    var response = await rootBundle.loadString('json_file/kuis.json');
    var object = json.decode(response)[NumberSoal]['key'];

    String soalnya = object.toString().toLowerCase().replaceAll(' ', '');
